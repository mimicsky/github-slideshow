...
| 20 Aug | 🔴 **KILLA CANNOT BE AVOIDED ON INTERCHANGE.** He **left the mall entirely and killed the player at the EMERCOM CHECKPOINT extract.** Reddit reports he is **far more aggressive in 1.1 than previously** — unclear if intentional or a bug. **"Walk past Killa" is dead advice — he hunts across the whole map, including extracts.** Every Interchange card must assume contact. | ✅ done |
...
|---|---|---|
| 2026-08-20 | desktop | **REQUEST 13 (Killa 1.1 aggression) DONE + MERGED.** Researched the "more aggressive in 1.1" claim: **not a bug — there's a documented 2026 AI overhaul.** *"Boss awareness expands with sound and line of sight; once triggered they reposition rather than anchor."* That's exactly the EMERCOM chase. **"Avoid his zone" is dead for every boss, not just Killa.** ⭐ **Answer on deferring Interchange: SPLIT IT.** `Supervisor` completes **on placement** so it can't be lost — **go now in a cheap kit.** The **books can be lost**, so defer them to 32+/36 and pair with the tech circuit. Also noted an alternative aim point *(neck)*, kept subordinate to their player-confirmed thorax rule. |
